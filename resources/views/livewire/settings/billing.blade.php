...
new class extends Component
{
    public function with(): array
    {
        ...
        return [
            ...
            'plan' => $plan,
            'productLimit' => $user->featureLimit('products'),
            'productUsage' => $user->featureUsage('products'),
            'scheduledLimit' => $plan?->getLimit('scheduled_launches'),
        ];
    }
                ...
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        <div class="flex items-center gap-2">
                            ...
                            @if ($subscribed)
                                <span class="badge border-emerald-500/30 bg-emerald-500/10 text-emerald-500"><span class="size-1.5 rounded-full bg-emerald-500"></span> Active</span>
                            @endif
                        </div>
            ...
            <div class="grid gap-5 p-5 sm:grid-cols-2">
                <div>
                    <div class="flex items-center justify-between text-[12.5px]">
                        <span class="font-medium text-muted">Products hunted</span>
                        <span class="text-subtle tabular-nums">{{ $productUsage }} / {{ is_null($productLimit) ? '∞' : $productLimit }}</span>
                    </div>
                    <div class="mt-2 h-1.5 overflow-hidden rounded-full bg-elevated">
                        <div class="h-full rounded-full bg-accent" style="width: {{ is_null($productLimit) || $productLimit == 0 ? ($productUsage > 0 ? 12 : 0) : min(100, round($productUsage / $productLimit * 100)) }}%"></div>
                    </div>
                </div>
                <div>
                    <div class="flex items-center justify-between text-[12.5px]">
                        <span class="font-medium text-muted">Scheduled launches</span>
                        <span class="text-subtle tabular-nums">{{ is_null($scheduledLimit) || $scheduledLimit < 0 ? 'Unlimited' : 'up to '.$scheduledLimit }}</span>
                    </div>
                    <div class="mt-2 h-1.5 overflow-hidden rounded-full bg-elevated">
                        <div class="h-full rounded-full bg-accent/60" style="width: {{ is_null($scheduledLimit) || $scheduledLimit < 0 ? 30 : 60 }}%"></div>
                    </div>
                </div>
            </div>
