...
new class extends Component
{
    ...
    public function open(string $id)
    {
        ...
        return $this->redirect($note->data['url'] ?? route('notifications'), navigate: true);
    }
}; ?>
    ...
    <div class="card mt-5 overflow-hidden">
        @forelse ($items as $note)
            @php
                ...
                $meta = match ($event) {
                    'ticket_assigned' => ['icon' => 'user', 'tone' => 'text-accent bg-accent-soft'],
                    'new_reply' => ['icon' => 'reply', 'tone' => 'text-sky-400 bg-sky-500/12'],
                    'ticket_resolved' => ['icon' => 'check-circle', 'tone' => 'text-emerald-400 bg-emerald-500/12'],
                    'sla_breach' => ['icon' => 'timer', 'tone' => 'text-rose-400 bg-rose-500/12'],
        ...
        @empty
            <div class="flex flex-col items-center px-6 py-20 text-center">
                <span class="grid size-14 place-items-center rounded-full bg-elevated text-accent"><x-icon name="bell" class="size-7" /></span>
                <h3 class="mt-5 text-lg font-semibold text-fg">{{ $filter === 'unread' ? 'No unread notifications' : "You're all caught up" }}</h3>
                <p class="mt-1.5 max-w-sm text-[14px] text-muted text-pretty">When a ticket is assigned to you or a customer replies, it'll show up here.</p>
            </div>
        @endforelse
    </div>
</div>
