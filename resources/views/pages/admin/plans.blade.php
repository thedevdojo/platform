...
<x-layouts.app title="Plans · Admin">
                ...
                @foreach ($plans as $plan)
                    <div wire:key="plan-{{ $plan->id }}" class="card flex flex-col p-5">
                        <div class="flex items-start justify-between">
                            ...
                            <span class="badge bg-emerald-500/10 text-emerald-500 border-emerald-500/20">Active</span>
                        </div>
                        ...
                        <div class="mt-4 flex items-baseline gap-1">
                            <span class="text-2xl font-extrabold tracking-tight text-fg tabular-nums">
                        ...
                        <div class="mt-4 grid grid-cols-2 gap-2">
                            <div class="rounded-lg border border-line bg-canvas-subtle px-3 py-2">
                                <p class="flex items-center gap-1.5 text-[11px] font-medium uppercase tracking-wider text-subtle">
                                    <x-icon name="rocket-launch" class="size-3.5" /> Products
                                </p>
                                <p class="mt-0.5 text-[14px] font-semibold text-fg">{{ $formatLimit($plan, 'products') }}</p>
                            </div>
                            <div class="rounded-lg border border-line bg-canvas-subtle px-3 py-2">
                                <p class="flex items-center gap-1.5 text-[11px] font-medium uppercase tracking-wider text-subtle">
                                    <x-icon name="calendar" class="size-3.5" /> Scheduled
                                </p>
                                <p class="mt-0.5 text-[14px] font-semibold text-fg">{{ $formatLimit($plan, 'scheduled_launches') }}</p>
                            </div>
                        </div>
