    ...
    <article class="mx-auto max-w-3xl px-5 pb-28 pt-12 sm:px-8">
        ...
        <header class="mt-7 animate-enter-up">
            ...
            <div class="mt-7 flex items-center gap-3 border-t border-line pt-6">
                <x-avatar :name="$post->user?->name ?? 'Relay'" :src="$post->user?->avatar" size="lg" />
                <div class="min-w-0">
                    <p class="text-[14px] font-semibold text-fg">{{ $post->user?->name ?? 'Relay Team' }}</p>
                    <p class="truncate text-[12.5px] text-subtle">{{ $authorTitle ?: 'Writing at Relay' }}</p>
                </div>
            </div>
        </header>
        ...
        <div class="mt-14 border-t border-line pt-10">
            <div class="card flex flex-col gap-4 p-6 sm:flex-row sm:items-center sm:gap-5">
                <x-avatar :name="$post->user?->name ?? 'Relay'" :src="$post->user?->avatar" size="xl" />
                <div class="min-w-0 flex-1">
                    <p class="text-[12px] font-semibold uppercase tracking-wider text-subtle">Written by</p>
                    <p class="mt-1 text-lg font-semibold tracking-tight text-fg">{{ $post->user?->name ?? 'Relay Team' }}</p>
