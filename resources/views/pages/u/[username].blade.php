...
@else
    @php
        ...
        $assignedCount = $user->assignedTickets()->count();
        $resolvedCount = $user->assignedTickets()
            ->whereIn('status', [\App\Enums\TicketStatus::Resolved, \App\Enums\TicketStatus::Closed])
            ->count();

        $activity = \App\Models\TicketEvent::where('user_id', $user->id)
            ->with('ticket')
            ->latest()
            ->take(6)
            ->get();
    ...
    <x-layouts.marketing :title="$user->name">
            ...
            <div class="relative mx-auto max-w-3xl px-5 pb-9 pt-20 sm:px-8 sm:pt-24">
                <div class="stagger flex flex-col items-center text-center sm:flex-row sm:items-end sm:text-left">
                    ...
                    <div class="mt-5 min-w-0 sm:mb-1 sm:ml-6 sm:mt-0">
                        <h1 class="font-display text-3xl font-semibold tracking-tight text-fg">{{ $user->name }}</h1>
        ...
        <div class="mx-auto max-w-3xl space-y-10 px-5 py-10 sm:px-8 sm:py-12">
            ...
            <div class="stagger grid grid-cols-3 gap-3">
                @foreach ([
                    ['Member since', $memberSince, 'calendar', 'text-accent'],
                    ['Tickets assigned', $assignedCount, 'inbox', 'text-violet-400'],
                    ['Resolved', $resolvedCount, 'check-circle', 'text-emerald-400'],
                ] as $stat)
