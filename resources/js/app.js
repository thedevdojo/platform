/**
 * Hunted front-end glue. Livewire 4 bundles Alpine, so we hook `alpine:init`
 * to register shared stores (theme, toasts).
 */

const THEME_KEY = 'hunted-theme';
...
// attributes onto the live document, and pages are server-rendered with the
// default light theme. The anti-flash <script> in <head> only runs on the
// first load (head scripts run once with wire:navigate), so without this a
// navigation would clobber a stored "dark" preference. The onSwap callback
// runs right after the new HTML is swapped in but before the browser paints.
